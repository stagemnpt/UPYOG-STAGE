...
public class PropertyQueryBuilder {
	...
	public final static String TOTAL_PROPERTY_REGISTERED_COUNT_WITH_WARD = "SELECT\r\n"
			...
			+ "	ept.created_time between extract(EPOCH from CURRENT_DATE) * 1000\r\n"
			+ "                             and extract(EPOCH from CURRENT_DATE + interval '1 day' - interval '1 second') * 1000\r\n"
			+ "                             and epa.ward_no!=''\r\n"
	...
	public final static String TOTAL_ASSEDPROPERTIES_COUNT_WITH_WARD = "SELECT \r\n" + "  CASE\r\n"
			...
			+ "JOIN eg_pt_address epa ON epp.id = epa.propertyid\r\n" + "WHERE epa.ward_no != ''\r\n"
			+ "  AND epaa.createdtime BETWEEN \r\n" + "      EXTRACT(EPOCH FROM CURRENT_DATE) * 1000 AND \r\n"
			+ "      EXTRACT(EPOCH FROM CURRENT_DATE + INTERVAL '1 day' - INTERVAL '1 second') * 1000\r\n"
			+ "GROUP BY epp.tenantid, epa.ward_no, \r\n" + "  CASE\r\n"
			+ "    WHEN epp.usagecategory IN ('OWNER_RESIDENTIAL', 'TENANT_RESIDENTIAL', 'AGRICULTURAL_WITHOUT_USE', 'APPURTENENT_LAND') THEN 'Residential'\r\n"
			+ "    WHEN epp.usagecategory IN ('OWNER_COMMERCIAL', 'TENANT_COMMERCIAL', 'AGRICULTURAL_COMMERCIAL_USE') THEN 'Commercial'\r\n"
			+ "    WHEN epp.usagecategory IN ('OWNER_MIXED', 'TENANT_MIXED') THEN 'Mixed Use'\r\n"
	...
	public final static String TOTAL_INTEREST_COLLECTED_WITH_WARD = "SELECT \r\n"
			...
			+ "  AND (\r\n"
			+ "    ep.createdtime BETWEEN EXTRACT(EPOCH FROM CURRENT_DATE) * 1000 AND \r\n"
			+ "                          EXTRACT(EPOCH FROM CURRENT_DATE + INTERVAL '1 day' - INTERVAL '1 second') * 1000\r\n"
			+ "    OR\r\n"
			+ "    ebv2.lastmodifieddate BETWEEN EXTRACT(EPOCH FROM CURRENT_DATE) * 1000 AND \r\n"
			+ "                               EXTRACT(EPOCH FROM CURRENT_DATE + INTERVAL '1 day' - INTERVAL '1 second') * 1000\r\n"
			+ "  )\r\n"
			+ "GROUP BY \r\n"
			+ "  epp.tenantid,\r\n"
	...
	public String getPropertySearchQuery(PropertyCriteria criteria, List<Object> preparedStmtList,
		...
		/*
		 * Condition to evaluate if owner is active. Inactive owners should never be
		 * shown in results
		 */
