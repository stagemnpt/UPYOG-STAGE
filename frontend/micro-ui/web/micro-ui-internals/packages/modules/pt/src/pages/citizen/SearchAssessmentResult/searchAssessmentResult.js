...
const AssessmentSearchResults = ({ template, header, actionButtonLabel, isMutation, isAmalgamation, onSelect, config, clearParams = () => {} }) => {
  const { t } = useTranslation();
  const modalRef = useRef();
  const { mobileNumber, propertyIds, oldPropertyIds, locality, city,doorNo,name,financialYear, PToffset } = Digit.Hooks.useQueryParams();
  let filters = {};
  ...
  const result = Digit.Hooks.pt.usePropertyAssessmentSearch(searchArgs);
  const consumerCode = result?.data?.Assessments?.map((a) => a.propertyId).join(",");
  ...
  const arr = isMutation ? result?.data?.Assessments?.filter((e) => e.status === "ACTIVE") : result?.data?.Assessments;

  const searchResults = arr?.map((assessment) => {
    let addr = assessment?.address || {};

    return {
      property_id: assessment?.propertyId,
      assessment_id: assessment?.assessmentNumber,
      financial_year: assessment?.financialYear,
      mode_of_payment: assessment?.modeOfPayment,
      owner_name: (assessment?.owners || [])?.map( o => o?.name ). join(","),
      property_address: [addr.doorNo || "", addr.buildingName || "", addr.street || "", addr.locality?.name || "", addr.city || ""]
        .filter((a) => a)
        .join(", "),
      total_due: payment[assessment?.propertyId]?.total_due || 0,
      bil_due__date: payment[assessment?.propertyId]?.bil_due__date || t("N/A"),
      status: t(assessment?.status),
    };
  });
...
AssessmentSearchResults.propTypes = {
  template: PropTypes.any,
  header: PropTypes.string,
  actionButtonLabel: PropTypes.string,
};

AssessmentSearchResults.defaultProps = {
  template: [],
  header: null,
